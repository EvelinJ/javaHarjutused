...
public class Peamurdja1_foor extends Application {
    ...
    public void start(Stage primaryStage) throws Exception {
        ...
        int count = 0;
        while (count < 10) {
            foor.vahetaPunast(); //paneb punase põlema
            foor.paus(0.5);
            foor.vahetaKollast(); //paneb kollase põlema
            foor.paus(0.5);
            foor.vahetaPunast(); // kustutab punase
            foor.vahetaKollast(); // kustutab kollase
            foor.vahetaRohelist(); // paneb rohelise põlema
            foor.paus(2);
            foor.vahetaRohelist(); // koos pausiga sama värv paneb vilkuma
            foor.paus(0.5);
            foor.vahetaRohelist();
            foor.paus(0.5);
            foor.vahetaRohelist();
